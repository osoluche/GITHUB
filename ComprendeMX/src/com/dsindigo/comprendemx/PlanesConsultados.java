...
public class PlanesConsultados extends Activity{
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			...
			try {
				...
				historial = json.get();
				JSONArray metodos = historial.getJSONArray("idPcsHist");
				
				ConectorSQL dbo = new ConectorSQL(getBaseContext());
				dbo.abrir();
				
				//Guardar en la DB nuevo historial
				for (int i = 0; i < metodos.length(); i++) {

					String values = metodos.getString(i);
					JSONObject appdata = new JSONObject(values);

					String planclase = appdata.getString("nombreplanclase");
					String idplancla = appdata.getString("id_pc");
					
					String verificador = dbo.consultarSolicitud(idplancla);
					if(Integer.parseInt(verificador) == 0)
					{
						dbo.registrarSolicitud(idplancla, planclase);
					}
				}

				//Mostrar registros guardados
				Cursor sc = dbo.obtenerSolicitudes();
				if(sc.moveToFirst())
				{
					do{
						...
						tableRows.setLayoutParams(new TableLayout.LayoutParams(LayoutParams.FILL_PARENT, LayoutParams.FILL_PARENT));
						...
						newText.setLayoutParams(new TableRow.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT));
								...
								public void onClick(View v) {
									...
									try {
										...
										if(idsolicitud.length() > 0)
										{
											...
											dbx.abrir();
											dbx.actualizarSolicitud(plancla, idsolicitud);
											dbx.cerrar();
							...
							if(auth.equals("1"))
							{
						        ...
						        solbtn.setOnClickListener(new View.OnClickListener() {
									...
									public void onClick(View v) {
										...
										Object planclase = v.getTag();
										String idpc = planclase.toString();
	...
	//Operaciones async
	public class ConsultarHistorial extends AsyncTask<String , String , JSONObject>{
		...
		protected JSONObject doInBackground(String... params) {
			...
			Log.d("Response => ", appdata.toString());
			return appdata;
		}	
	}
	...
	public class ConsultarSolicitud extends AsyncTask<String, String, String>{
		...
		protected String doInBackground(String... params) {
			ConectorJson cnx = new ConectorJson();
			String respuesta = cnx.ConsultarSolicitud(params[0]);
			return respuesta;
		}
	}
	...
	public class ConsultarAutorizacion extends AsyncTask<String, String, String>{
	...
	public class DescargarPlan extends AsyncTask<String, String, String>{
		...
		protected String doInBackground(String... params) {
			...
			Log.d("tracemx", "Descargando... ");
			
			ConectorJson cnx = new ConectorJson();
			JSONObject jsondata = cnx.GetPlayById(Integer.parseInt(params[0]));
			...
			try {
				 titulo = jsondata.getString("nbResource");
				 idplan = jsondata.getString("idplanclase");
				 enlace = jsondata.getString("txDownloadUrl");
			...
			if(responsedw)
			{
				...
				String location = Environment.getExternalStorageDirectory() + File.separator + "ComprendeMX";
				String filezip  = Environment.getExternalStorageDirectory() + File.separator + "ComprendeMX" + File.separator + fileLoca + ".zip";
				
				try {
					ZipFile zipf = new ZipFile(filezip);
				...
				/* Poner plan disponible nuevamente */
				boolean resetDw = conn.actualizarEstadoSolicitud(idplan);
				if(resetDw)
				{
					Log.d("dbmx", "Actualizado");
				}
				
				boolean existe = conn.preguntarPlan(idplan);
				
				if(!existe)
				{
					conn.insertarPlan(idplan, titulo, fileLoca);	
				}
				else
				{
					conn.actualizarExistenciaPlan(idplan);
				}
